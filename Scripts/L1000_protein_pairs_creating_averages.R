...
save(L1000_prot_pair_averages, file="/mnt/storage/scratch/hw15842/L1000/Results/L1000_prot_pair_averages.rdata")
...
## Do the same for the the TOTAL protein pairs 
...
func2 <- function(dataframe){
	...
	df <- TOTAL_prot_pairs_list_dfs[[dataframe]]
	df$Z_score_adjusted <- ifelse(grepl("CGS001",df$sig_info), (df$Z * (-1)), df$Z)
...
TOTAL_L1000_prot_pair_averages <- lapply(1:length(TOTAL_prot_pairs_list_dfs), func2)
TOTAL_L1000_prot_pair_averages <- ldply(TOTAL_L1000_prot_pair_averages, data.table)




save(TOTAL_L1000_prot_pair_averages, file="/mnt/storage/scratch/hw15842/repo/L1000_protein_pair_trait_MR/Data/TOTAL_L1000_prot_pair_averages.rdata")
